	...
	while(!fin.eof()){
		...
		if (state == open){
			if (buffer == "csed"){
				...
				if (block[1] == "readfile")
                    blocks[number] = makeWorker(ReadFactory, blockStr[2]);
                if (block[1] == "writefile")
                    blocks[number] = makeWorker(WriteFactory, blockStr[2]);
                if (block[1] == "grep")
                    blocks[number] = makeWorker(GrepFactory, blockStr[2]);
                if (block[1] == "sort")
                    blocks[mumber] = makeWorker(SortFactory, blockStr[2]);
                if (block[1] == "replace")
                    blocks[number] = makeWorker(ReplacecFactory, blockStr[2]);
                if (block[1] == "dump")
                    blocks[number] = makeWorker(DumpFactory, blockStr[2]);
